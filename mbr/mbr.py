...
def main():
	...
	# current_sector_size = struct.unpack_from("<I",table,12)[0]
	parse_EBR_entry()
...
def read_sectors(fd, sector,count =  1):
	fd.seek(sector*512)
	# 섹터에 512를 곱하는 이유는 섹터의 크기가 512이라서인가
	# 섹터는 0부터 시작이다.
...
def print_table_entry(table):
	...
	# print("FileSystem :",table[1],table[2],table[3])
...
def parse_EBR_entry():
	...
	if chk_ebr_end(table2):
		...
		print(ebr_start_sector)
		print(struct.unpack_from("<I",table2,8)[0])
